...
export const SceneSegmentForm: React.FC<ISceneSegmentForm> = ({
    ...
    const initialValues = useMemo(
        () => ({
            title: segment?.title ?? "",
            start_seconds: segment?.start_seconds ?? Math.round(getPlayerPosition() ?? 0),
            end_seconds: segment?.end_seconds ?? Math.round(getPlayerPosition() ?? 0) + 60,
        }),
        [segment]
    );
    ...
    function renderStartTimeField() {
        ...
        const title = intl.formatMessage({ id: "time_start", defaultMessage: "Start Time" });
        const control = (
            <DurationInput
            ...
            <div className="form-container px-3">
                <h4>
                    {isNew ? (
                        ...
                        <FormattedMessage id="actions.edit_segment" defaultMessage="Edit Segment" />
                    )}
                </h4>
                {renderTitleField()}
