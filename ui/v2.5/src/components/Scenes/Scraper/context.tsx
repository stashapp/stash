...
import { IScraperSource, SCRAPER_PREFIX, STASH_BOX_PREFIX } from "./constants";

export interface ISceneScraperContextState {
  ...
  currentSource?: IScraperSource;
  searchResults: Record<string, ISceneQueryResult>;
  setCurrentSource: (src?: IScraperSource) => void;
...
export const SceneScraperStateContext = React.createContext<ISceneScraperContextState>(
  {
...
export const SceneScraperContext: React.FC = ({ children }) => {
  ...
  const [currentSource, setCurrentSource] = useState<IScraperSource>();
  ...
  useEffect(() => {
    ...
    const stashboxSources: IScraperSource[] = stashBoxes.map((s, i) => ({
    ...
    const scraperSources: IScraperSource[] = scrapers
      .filter((s) =>
      ...
      .map((s) => ({
        ...
        displayName: s.name,
        supportQuery: s.scene?.supported_scrapes.includes(GQL.ScrapeType.Name),
        supportFragment: s.scene?.supported_scrapes.includes(
  ...
  return (
    <SceneScraperStateContext.Provider
      value={{
