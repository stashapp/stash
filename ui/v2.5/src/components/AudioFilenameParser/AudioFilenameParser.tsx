...
import { IParserInput, ParserInput } from "../SceneFilenameParser/ParserInput";
import { ParserField } from "../SceneFilenameParser/ParserField";
import { AudioParserResult, AudioParserRow } from "./AudioParserRow";
...
export const AudioFilenameParser: React.FC = () => {
  const intl = useIntl();
  const Toast = useToast();
  const [parserResult, setParserResult] = useState<AudioParserResult[]>([]);
  ...
  const [updateAudios] = useAudiosUpdate(getAudiosUpdateData());
  ...
  const parseResults = useCallback(
    (
      results: GQL.ParseAudioFilenamesQuery["parseAudioFilenames"]["results"]
    ) => {
      if (results) {
        const result = results
          .map((r) => {
            return new AudioParserResult(r);
          })
          .filter((r) => !!r) as AudioParserResult[];
  ...
  const parseAudioFilenames = useCallback(() => {
    setParserResult([]);
    setIsLoading(true);
    ...
    queryParseAudioFilenames(parserFilter, parserInputData)
      .then((response) => {
        const result = response?.data?.parseAudioFilenames;
        if (result) {
  ...
  }, [parserInput, parseAudioFilenames, prevParserInput]);
  ...
  function getAudiosUpdateData() {
    return parserResult
      .filter((result) => result.isChanged())
      .map((result) => result.toAudioUpdateInput());
  }
    ...
    try {
      ...
      Toast.success(
        intl.formatMessage(
          ...
          { entity: intl.formatMessage({ id: "audios" }).toLocaleLowerCase() }
        )
      );
    } catch (e) {
      Toast.error(e);
  ...
  function onChange(audio: AudioParserResult, changedAudio: AudioParserResult) {
    const newResult = [...parserResult];

    const index = newResult.indexOf(audio);
    newResult[index] = changedAudio;
      ...
      <>
        <div className="scene-parser-results">
          <Table>
            ...
            <tbody>
              {parserResult.map((audio) => (
                <AudioParserRow
                  audio={audio}
                  key={audio.id}
                  onChange={(changedAudio) => onChange(audio, changedAudio)}
  ...
  return (
    <Card id="parser-container" className="col col-sm-9 mx-auto">
      <h4>
        {intl.formatMessage({ id: "config.tools.audio_filename_parser.title" })}
      </h4>
      <ParserInput
        input={parserInput}
...
export default AudioFilenameParser;
