...
export const IdentifyDialog: React.FC<IIdentifyDialogProps> = ({
  ...
  function getDefaultOptions(): GQL.IdentifyGalleryMetadataOptionsInput {
    return {
      fieldOptions: [
        {
          field: "title",
  ...
  const [options, setOptions] =
    useState<GQL.IdentifyGalleryMetadataOptionsInput>(getDefaultOptions());
  ...
  const { data: scraperData, error: scraperError } = useListGalleryScrapers();
  ...
  const selectionStatus = useMemo(() => {
    if (selectedIds) {
      return (
        <Form.Group id="selected-identify-ids">
          <FormattedMessage
            id="config.tasks.identify.identifying_galleriesz"
            values={{
              num: selectedIds.length,
              gallery: intl.formatMessage(
                {
    ...
    const message = paths.length ? (
      <div>
        <FormattedMessage id="config.tasks.identify.identifying_galleries_from_paths" />
        :
        <ul>
          {paths.map((p) => (
            <li key={p}>{p}</li>
      ...
      <span>
        <FormattedMessage
          id="config.tasks.identify.identifying_galleries"
          values={{
  ...
  useEffect(() => {
    ...
    const { identifyGallery: identifyGalleryDefaults } =
      configData.configuration.defaults;

    if (identifyGalleryDefaults) {
      const mappedSources = identifyGalleryDefaults.sources
        .map((s) => {
      ...
      if (identifyGalleryDefaults.options) {
        const defaultOptions = withoutTypename(identifyGalleryDefaults.options);
  ...
  function makeIdentifyGalleryInput(): GQL.IdentifyGalleryMetadataInput {
    return {
  ...
  function makeDefaultIdentifyGalleryInput() {
    const ret = makeIdentifyGalleryInput();
    const { galleryIDs, paths: _paths, ...withoutSpecifics } = ret;
    return withoutSpecifics;
  }

  async function onIdentifyGallery() {
    try {
      await mutateMetadataIdentifyGallery(makeIdentifyGalleryInput());
  ...
  async function setAsDefault() {
    try {
      ...
      await configureDefaults({
        variables: {
          input: {
            identifyGallery: makeDefaultIdentifyGalleryInput(),
          },
        },
      });
      ...
      Toast.success(
        intl.formatMessage(
          ...
          { action: intl.formatMessage({ id: "actions.identify_galleries" }) }
        )
      );
    } catch (e) {
      Toast.error(e);
  ...
  return (
    <ModalComponent
      ...
      show
      icon={faCogs}
      header={intl.formatMessage({ id: "actions.identify_galleries" })}
      accept={{
        onClick: onIdentifyGallery,
        text: intl.formatMessage({ id: "actions.identify_galleries" }),
      }}
      cancel={{
        onClick: () => onClose(),
