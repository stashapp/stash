...
import {
  useAudioDecrementO,
  useAudioDecrementPlayCount,
  useAudioIncrementO,
  useAudioIncrementPlayCount,
  useAudioResetO,
  useAudioResetPlayCount,
  useAudioResetActivity,
...
interface IAudioHistoryProps {
  audio: GQL.AudioDataFragment;
}

export const AudioHistoryPanel: React.FC<IAudioHistoryProps> = ({ audio }) => {
  const intl = useIntl();
  ...
  const [incrementPlayCount] = useAudioIncrementPlayCount();
  const [decrementPlayCount] = useAudioDecrementPlayCount();
  const [clearPlayCount] = useAudioResetPlayCount();
  const [incrementOCount] = useAudioIncrementO(audio.id);
  const [decrementOCount] = useAudioDecrementO(audio.id);
  const [resetO] = useAudioResetO(audio.id);
  const [resetResume] = useAudioResetActivity(audio.id, true, false);
  const [resetDuration] = useAudioResetActivity(audio.id, false, true);
  ...
  const playHistory = (audio.play_history ?? []).filter(
    (h) => h != null
  ) as string[];
  const oHistory = (audio.o_history ?? []).filter((h) => h != null) as string[];
    ...
    <div>
      ...
      <div className="play-history">
        ...
        <History
          ...
          unknownDate={audio.created_at}
          onRemove={(t) => handleDeletePlayDate(t)}
        />
        <dl className="details-list">
          <TextField
            id="media_info.play_duration"
            value={TextUtils.secondsToTimestamp(audio.play_duration ?? 0)}
          />
        </dl>
      </div>
...
export default AudioHistoryPanel;
