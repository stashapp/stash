...
export type GroupAudioIndexMap = Map<string, number | undefined>;

export interface IGroupEntry {
  group: Group;
  audio_index?: GQL.InputMaybe<number> | undefined;
}
...
export const AudioGroupTable: React.FC<IProps> = (props) => {
  ...
  const updateFieldChanged = (index: number, audioIndex: number | null) => {
  ...
  return (
    <div className={cx("group-table", { "no-groups": !value.length })}>
      <Row className="group-table-header">
        <Col xs={9}></Col>
        <Form.Label column xs={3} className="group-audio-number-header">
          {intl.formatMessage({ id: "group_audio_number" })}
        </Form.Label>
      </Row>
      {renderTableData()}
    </div>
