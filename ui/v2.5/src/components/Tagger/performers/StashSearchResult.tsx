...
interface IStashSearchResultProps {
  ...
  endpoint: string;
  onPerformerTagged: (performer: Pick<GQL.SlimPerformerDataFragment, 'id'> & Partial<Omit<GQL.SlimPerformerDataFragment, 'id'>>) => void;
}
...
const StashSearchResult: React.FC<IStashSearchResultProps> = ({
  ...
  const [modalPerformer, setModalPerformer] = useState<IStashBoxPerformer | undefined>();
  const [selectedPerformer, setSelectedPerformer] = useState<IStashBoxPerformer | undefined>();
  ...
  const handleSave = async () => {
    if (selectedPerformer) {
      ...
      if (!res.data?.performerUpdate)
        ...
        onPerformerTagged(performer);
      setSaveState("");
    }
  };

  const performers = (
    stashboxPerformers.map(p => {
  ...
  const handlePerformerSelect = (image: number) => {
    ...
    if (modalPerformer?.stash_id)
      setPerformerImage({ ...performerImage, [modalPerformer.stash_id]: image });
  ...
  return (
    <>
      ...
      <div className="PerformerTagger-performer-search">
        { performers }
      </div>
