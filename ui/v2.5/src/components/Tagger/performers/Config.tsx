        ...
        <Card>
          <div className="row">
            ...
            <div className="col-md-6">
              <Form.Group
                controlId="excluded-performer-fields"
              >
                <h6>Excluded fields:</h6>
                <span>
                  { excludedFields.length > 0 ? (
                    excludedFields.map(f => <Badge variant="secondary" className="tag-item">{TextUtils.capitalize(f)}</Badge>)
                  ) : "No fields are excluded" }
                </span>
                <Form.Text>These fields will not be changed when updating performers.</Form.Text>
                <Button onClick={() => setShowExclusionModal(true)} className="mt-2">Edit Excluded Fields</Button>
              </Form.Group>
              <Form.Group
