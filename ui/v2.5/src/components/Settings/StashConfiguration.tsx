...
const Stash: React.FC<IStashProps> = PatchComponent(
  "Stash",
  ({ index, stash, onSave, onEdit, onDelete }: IStashProps) => {
...
const StashConfiguration: React.FC<IStashConfigurationProps> = PatchComponent(
  "StashConfiguration",
  ({ stashes, setStashes }: IStashConfigurationProps) => {
...
export const StashSetting: React.FC<IStashSetting> = PatchComponent(
  "StashSetting",
  ({ value, onChange }: IStashSetting) => {
    return (
      <SettingSection
        id="stashes"
        headingID="library"
