...
export const SettingsInterfacePanel: React.FC = PatchComponent(
  ...
  () => {
    ...
    const massageMenuItems = useCallback((menuItems: string[]) => {
      return menuItems.map((item) => (item === "movies" ? "groups" : item));
    }, []);
    ...
    function validateJavascriptString(v: string) {
      if (!v) return;
      try {
        new Function(v); // eslint-disable-line @typescript-eslint/no-implied-eval
      } catch (e) {
        throw new Error(
          intl.formatMessage(
      ...
      <>
        ...
        <SettingSection headingID="config.ui.scene_player.heading">
          ...
          <BooleanSetting
            ...
            onChange={(v) =>
              saveInterface({ continuePlaylistDefault: v })
            }
          />
          ...
          <ModalSetting<number>
            ...
            renderValue={(v) => (
              <span>{TextUtils.secondsToTimestamp(v ?? 0)}</span>
            )}
          />
        ...
        <SettingSection headingID="config.ui.detail.heading">
          <div className="setting-group">
            ...
            <BooleanSetting
              ...
              onChange={(v) =>
                saveUI({
                  enablePerformerBackgroundImage: v,
                })
              }
            />
            <BooleanSetting
