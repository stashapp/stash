...
export const SettingStateContext = React.createContext<ISettingsContextState | null>(null);
...
export const SettingsContext: React.FC = PatchComponent(
  "SettingsContext",
  ({ children }) => {
    ...
    const [pendingGeneral, setPendingGeneral] = useState<GQL.ConfigGeneralInput>();
    const [updateGeneralConfig] = useConfigureGeneral();

    const [iface, setIface] = useState<GQL.ConfigInterfaceInput>({});
    const [pendingInterface, setPendingInterface] = useState<GQL.ConfigInterfaceInput>();
    const [updateInterfaceConfig] = useConfigureInterface();

    const [defaults, setDefaults] = useState<GQL.ConfigDefaultSettingsInput>({});
    const [pendingDefaults, setPendingDefaults] = useState<GQL.ConfigDefaultSettingsInput>();
    const [updateDefaultsConfig] = useConfigureDefaults();

    const [scraping, setScraping] = useState<GQL.ConfigScrapingInput>({});
    const [pendingScraping, setPendingScraping] = useState<GQL.ConfigScrapingInput>();
    ...
    const saveGeneralConfig = useDebounce(async (input: GQL.ConfigGeneralInput) => {
      try {
        setUpdateSuccess(undefined);
        await updateGeneralConfig({ variables: { input } });
    ...
    function saveGeneral(input: Partial<GQL.ConfigGeneralInput>) {
      ...
      setGeneral({ ...general, ...input });
      setPendingGeneral((current) => (current ? { ...current, ...input } : input));
    }

    const saveInterfaceConfig = useDebounce(async (input: GQL.ConfigInterfaceInput) => {
      try {
        setUpdateSuccess(undefined);
        await updateInterfaceConfig({ variables: { input } });
    ...
    function saveInterface(input: Partial<GQL.ConfigInterfaceInput>) {
      ...
      setIface({ ...iface, ...input });
      setPendingInterface((current) => (current ? { ...current, ...input } : input));
    }

    const saveDefaultsConfig = useDebounce(async (input: GQL.ConfigDefaultSettingsInput) => {
      try {
        setUpdateSuccess(undefined);
        await updateDefaultsConfig({ variables: { input } });
    ...
    function saveDefaults(input: Partial<GQL.ConfigDefaultSettingsInput>) {
      ...
      setDefaults({ ...defaults, ...input });
      setPendingDefaults((current) => (current ? { ...current, ...input } : input));
    }

    const saveScrapingConfig = useDebounce(async (input: GQL.ConfigScrapingInput) => {
      try {
        setUpdateSuccess(undefined);
        await updateScrapingConfig({ variables: { input } });
    ...
    function saveScraping(input: Partial<GQL.ConfigScrapingInput>) {
      ...
      setScraping({ ...scraping, ...input });
      setPendingScraping((current) => (current ? { ...current, ...input } : input));
    }

    const saveDLNAConfig = useDebounce(async (input: GQL.ConfigDlnaInput) => {
      try {
        setUpdateSuccess(undefined);
        await updateDLNAConfig({ variables: { input } });
    ...
    function saveDLNA(input: Partial<GQL.ConfigDlnaInput>) {
      ...
      setDLNA({ ...dlna, ...input });
      setPendingDLNA((current) => (current ? { ...current, ...input } : input));
    }
    ...
    const saveUIConfig = useDebounce(async (input: Partial<IUIConfig>) => {
      try {
        setUpdateSuccess(undefined);
        await updateUIConfig({ variables: { partial: input as UIConfigInput } });
    ...
    function saveUI(input: IUIConfig) {
      ...
      setUI({ ...ui, ...input });
      setPendingUI((current) => ({ ...current, ...input }));
    }

    function setAdvancedMode(value: boolean) {
      saveUI({ advancedMode: value });
    }
    ...
    const savePluginConfig = useDebounce(async (input: PluginConfigs) => {
      try {
        ...
        for (const pluginID in input) {
          await updatePluginConfig({ variables: { plugin_id: pluginID, input: input[pluginID] } });
        }
    ...
    function savePluginSettings(pluginID: string, input: Record<string, unknown>) {
      if (!plugins) return;

      setPlugins({ ...plugins, [pluginID]: input });
      setPendingPlugins((current) => (current ? { ...current, [pluginID]: input } : { ...plugins, [pluginID]: input }));
    }
