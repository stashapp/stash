...
class SkipButtonPlugin extends VideoJS.getPlugin('plugin') {
  ...
  public setForwardHandler(handler?: () => void) {
    ...
    if (handler !== undefined)
      this.player.addClass('vjs-skip-buttons-next');
    else
      this.player.removeClass('vjs-skip-buttons-next');
  }

  public setBackwardHandler(handler?: () => void) {
    this.onPrevious = handler;
    if (handler !== undefined)
      this.player.addClass('vjs-skip-buttons-prev');
    else
      this.player.removeClass('vjs-skip-buttons-prev');
  }
  ...
  ready() {
    ...
    this.player.controlBar.addChild('skipButton', {
      direction: 'forward',
      parent: this,
    }, 1);
...
class SkipButton extends Button {
  ...
  handleClick() {
    if (this.direction === 'forward')
      this.parentPlugin.handleForward();
    else
      this.parentPlugin.handleBackward();
  }
}
