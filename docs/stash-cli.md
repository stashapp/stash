...
TUI 使用 Vim 风格 normal 模式。`h/j/k/l` 或方向键移动封面网格，`enter` 播放当前选中的 scene，`space` 显示或隐藏当前 scene 的详情/编辑面板。按 `:` 进入底部命令输入栏，`esc` 取消输入，`:q` 退出。

详情/编辑面板支持：

- `r`：编辑当前 scene 的 rating，范围 `0..100`。
- `d`：删除当前 scene；需要再按 `y` 确认。
- `j/k`：在 performer 列表中移动焦点。
- `R`：编辑当前 performer 的 rating，范围 `0..100`。
- `esc` 或 `space`：关闭详情/编辑面板。
...
- `enter`：使用 `ffplay_path` 播放当前选中的 scene primary file。`ffplay` 正常退出后，stash-cli 会给该 scene 增加一次 view history，用作播放次数记录。
- `/help`：显示命令列表。
- `:q`：退出。
