...
func TestSceneQueryStudioTags(t *testing.T) {
	allDepth := -1
	...
	tests := []struct {
	...
	}{
		{
			...
			nil,
			&models.SceneFilterType{
				StudioTags: &models.HierarchicalMultiCriterionInput{
					Value: []string{
		...
		{
			"excludes studio tag tagIdx2WithStudio",
			nil,
			&models.SceneFilterType{
				StudioTags: &models.HierarchicalMultiCriterionInput{
					Modifier: models.CriterionModifierExcludes,
					Value:    []string{strconv.Itoa(tagIDs[tagIdx2WithStudio])},
				},
			},
			nil,
			[]int{sceneIdxWithTwoStudioTag},
			false,
		},
		{
			"excludes sub-tags",
		...
		},
		{
			...
			[]int{sceneIdx1WithStudio},
			[]int{sceneIdxWithStudioTag},
			false,
		},
		{
			"not null",
