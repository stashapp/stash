...
func (qb *StudioStore) Create(ctx context.Context, newObject *models.Studio) error {
	...
	if newObject.Aliases.Loaded() {
		if err := studio.EnsureAliasesUnique(ctx, newObject.ID, newObject.Aliases.List(), qb); err != nil {
			return err
		}
...
func (qb *StudioStore) UpdatePartial(ctx context.Context, id int, partial models.StudioPartial) (*models.Studio, error) {
	...
	if partial.Aliases != nil {
		if err := studio.EnsureAliasesUnique(ctx, partial.ID, partial.Aliases.Values, qb); err != nil {
			return nil, err
		}

		if err := studiosAliasesTableMgr.modifyJoins(ctx, id, partial.Aliases.Values, partial.Aliases.Mode); err != nil {
			return nil, err
		}
	}

	if partial.StashIDs != nil {
		if err := studiosStashIDsTableMgr.modifyJoins(ctx, id, partial.StashIDs.StashIDs, partial.StashIDs.Mode); err != nil {
			return nil, err
		}
	}
