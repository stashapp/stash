...
func (l *PerformerLoader) Load(key int) (*Performer, error) {
...
func (l *PerformerLoader) LoadThunk(key int) func() (*Performer, error) {
	l.mu.Lock()
	if it, ok := l.cache[key]; ok {
		l.mu.Unlock()
		return func() (*Performer, error) {
			return it, nil
		}
	}
	if l.batch == nil {
...
func (l *PerformerLoader) LoadAll(keys []int) ([]*Performer, []error) {
	results := make([]func() (*Performer, error), len(keys))

	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}

	performers := make([]*Performer, len(keys))
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *PerformerLoader) LoadAllThunk(keys []int) func() ([]*Performer, []error) {
	results := make([]func() (*Performer, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *PerformerLoader) Prime(key int, value *Performer) bool {
	l.mu.Lock()
	var found bool
...
func (l *PerformerLoader) unsafeSet(key int, value *Performer) {
	if l.cache == nil {
		l.cache = map[int]*Performer{}
	}
	l.cache[key] = value
}
