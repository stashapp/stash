...
func (l *StudioLoader) Load(key int) (*Studio, error) {
...
func (l *StudioLoader) LoadThunk(key int) func() (*Studio, error) {
	l.mu.Lock()
	if it, ok := l.cache[key]; ok {
		l.mu.Unlock()
		return func() (*Studio, error) {
			return it, nil
		}
	}
	if l.batch == nil {
...
func (l *StudioLoader) LoadAll(keys []int) ([]*Studio, []error) {
	results := make([]func() (*Studio, error), len(keys))

	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}

	studios := make([]*Studio, len(keys))
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *StudioLoader) LoadAllThunk(keys []int) func() ([]*Studio, []error) {
	results := make([]func() (*Studio, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *StudioLoader) Prime(key int, value *Studio) bool {
	l.mu.Lock()
	var found bool
...
func (l *StudioLoader) unsafeSet(key int, value *Studio) {
	if l.cache == nil {
		l.cache = map[int]*Studio{}
	}
	l.cache[key] = value
}
