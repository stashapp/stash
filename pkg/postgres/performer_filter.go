	...
	return func(ctx context.Context, f *filterBuilder) {
		...
		INNER JOIN studio ON {primaryTable}.studio_id = studio.item_id`
			...
			if len(studios.Excludes) > 0 {
				excludeValuesClause, err := getHierarchicalValues(ctx, studios.Excludes, studioTable, "", "parent_id", "child_id", studios.Depth, false)
				if err != nil {
					f.setError(err)
					return
				}
...
func (qb *performerFilterHandler) groupsCriterionHandler(groups *models.HierarchicalMultiCriterionInput) criterionHandlerFunc {
	return func(ctx context.Context, f *filterBuilder) {
		if groups != nil {
			...
			for _, value := range groups.Value {
				id, err := strconv.Atoi(value)
				if err != nil {
					return
				}

				valuesClauses = append(valuesClauses, fmt.Sprintf("%d", id))
			}
			valuesClause := strings.Join(valuesClauses, ",")
			...
			if depthVal == 0 {
				...
				f.addWith("group_values(id) AS (VALUES (" + valuesClause + "))")
...
)`, valuesClause, depthCondition)

				f.addRecursiveWith(hierarchyQuery)
