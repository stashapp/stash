...
func (qb *TagStore) QueryForAutoTag(ctx context.Context, words []string) ([]*models.Tag, error) {
	...
	for _, w := range words {
		ww := w + "%"
		whereClauses = append(whereClauses, "tags.name ILIKE ?")
		args = append(args, ww)

		// include aliases
		whereClauses = append(whereClauses, "tag_aliases.alias ILIKE ?")
		args = append(args, ww)
	}

	whereOr := "(" + strings.Join(whereClauses, " OR ") + ")"
	where := strings.Join([]string{
		"tags.ignore_auto_tag = false",
		whereOr,
	}, " AND ")
...
func (qb *TagStore) getTagSort(query *queryBuilder, findFilter *models.FindFilterType) (string, []string, error) {
	...
	default:
		var add string
		add, group = getSort(sort, direction, "tags")
		sortQuery += add
	}
	...
	group = append(group, "tags.name", "tags.id")
...
func (qb *TagStore) Merge(ctx context.Context, source []int, destination int) error {
	...
	AND NOT EXISTS(SELECT 1 FROM `+table+` o WHERE o.`+idColumn+` = `+table+`.`+idColumn+` AND o.tag_id = $1)`,
			...
			if err != nil && !qb.repository.isConstraintError(err) {
				return err
			}
		}
	...
	// Merge StashIDs - insert non-conflicting source StashIDs as new rows with destination tag_id
	_, err = dbWrapper.Exec(ctx, `INSERT INTO tag_stash_ids (tag_id, endpoint, stash_id, updated_at)
	SELECT ?, endpoint, stash_id, updated_at FROM tag_stash_ids WHERE tag_id IN `+inBinding+` ON CONFLICT DO NOTHING`, args...)
	if err != nil {
		return err
	}

	// Delete all source StashIDs (including those that conflicted and weren't inserted)
...
parents AS (
	SELECT t.id AS parent_id, t.id AS child_id, t.name::text as path FROM tags t WHERE t.id = ?
	UNION
