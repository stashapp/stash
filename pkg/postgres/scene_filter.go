...
func (qb *sceneFilterHandler) addSceneFilesTable(f *filterBuilder) {
	f.addLeftJoin(scenesFilesTable, "", "scenes_files.scene_id = scenes.id AND scenes_files.\"primary\" = true")
}
...
func (qb *sceneFilterHandler) isMissingCriterionHandler(isMissing *string) criterionHandlerFunc {
	return func(ctx context.Context, f *filterBuilder) {
		if isMissing != nil && *isMissing != "" {
			...
			case "date":
				f.addWhere(`scenes.date IS NULL`)
			case "tags":
	...
	h := stringListCriterionHandlerBuilder{
		...
		excludeHandler: func(f *filterBuilder, criterion *models.StringCriterionInput) {
			excludeClause := `scenes.id NOT IN (
				...
				WHERE video_captions.language_code ILIKE ?
			)`
...
GROUP BY performers_scenes.scene_id HAVING SUM(performers.favorite) = false)`, "nofaves", "scenes.id = nofaves.id")
