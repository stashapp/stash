...
type rawRowsWithCount struct {
	ID          int             `db:"id"`
	TotalCount  sql.NullInt64   `db:"total_count"`
	TotalSize   sql.NullFloat64 `db:"total_size"`
	TotalCustom sql.NullFloat64 `db:"total_custom"` // Misc selectors
}

type RowsWithCounts struct {
	IDs         []int
...
func (r *repository) runIdsWithCount(ctx context.Context, query string, args []interface{}) (*RowsWithCounts, error) {
	var result []rawRowsWithCount

	if err := dbWrapper.Select(ctx, &result, query, args...); err != nil && !errors.Is(err, sql.ErrNoRows) {
		return nil, fmt.Errorf("running query: %s [%v]: %w", query, args, err)
	}
	...
	rows.IDs = make([]int, len(result))
	for i, row := range result {
		rows.IDs[i] = row.ID
	}

	rows.TotalCount = result[0].TotalCount
	rows.TotalSize = result[0].TotalSize
	rows.TotalCustom = result[0].TotalCustom
...
func (r *repository) executeFindQuery(ctx context.Context, body string, args []interface{}, sort string, pagination string, whereClauses []string, havingClauses []string, withClauses []string, groupByClauses []string, recursiveWith bool) ([]int, int, error) {
	...
	var err error
	if obj, err = r.runIdsWithCount(ctx, combinedQuery, args); err != nil {
		return nil, 0, err
	}

	return obj.IDs, int(obj.TotalCount.Int64), err
}
...
func (r *filesRepository) getMany(ctx context.Context, ids []int, primaryOnly bool) ([][]models.FileID, error) {
	...
	if primaryOnly {
		primaryClause = ` AND "primary" = true`
	}
...
func (r *repository) isConstraintError(err error) bool {
