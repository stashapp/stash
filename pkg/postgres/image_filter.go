...
GROUP BY performers_images.image_id HAVING SUM(performers.favorite) = false)`, "nofaves", "images.id = nofaves.id")
...
func (qb *imageFilterHandler) performerAgeCriterionHandler(performerAge *models.IntCriterionInput) criterionHandlerFunc {
	return func(ctx context.Context, f *filterBuilder) {
		if performerAge != nil {
			...
			ageCalc := "EXTRACT(YEAR FROM AGE(images.date, performers.birthdate))"
