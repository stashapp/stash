		...
		if c != nil {
			if modifier := c.Modifier; c.Modifier.IsValid() {
				...
				case models.CriterionModifierEquals:
					f.addWhere(column+" ILIKE ?", c.Value)
				case models.CriterionModifierNotEquals:
					f.addWhere(column+" NOT ILIKE ?", c.Value)
				case models.CriterionModifierMatchesRegex:
					if _, err := regexp.Compile(c.Value); err != nil {
						f.setError(err)
						return
					}
					f.addWhere(fmt.Sprintf("(%s IS NOT NULL AND regex_match(%[1]s, ?))", column), c.Value)
				case models.CriterionModifierNotMatchesRegex:
					if _, err := regexp.Compile(c.Value); err != nil {
						f.setError(err)
						return
					}
					f.addWhere(fmt.Sprintf("(%s IS NULL OR NOT regex_match(%[1]s, ?))", column), c.Value)
	...
	return func(ctx context.Context, f *filterBuilder) {
		...
		if c != nil {
			if modifier := c.Modifier; c.Modifier.IsValid() {
				...
				case models.CriterionModifierEquals:
					f.addWhere(columnCast+" ILIKE ?", c.Value)
				case models.CriterionModifierNotEquals:
					f.addWhere(columnCast+" NOT ILIKE ?", c.Value)
				case models.CriterionModifierMatchesRegex:
					if _, err := regexp.Compile(c.Value); err != nil {
						f.setError(err)
						return
					}
					f.addWhere(fmt.Sprintf("(%s IS NOT NULL AND regex_match(%s, ?))", column, columnCast), c.Value)
				case models.CriterionModifierNotMatchesRegex:
					if _, err := regexp.Compile(c.Value); err != nil {
						f.setError(err)
						return
					}
					f.addWhere(fmt.Sprintf("(%s IS NULL OR NOT regex_match(%s, ?))", column, columnCast), c.Value)
				case models.CriterionModifierIsNull:
		...
		if c != nil {
			...
			if modifier := c.Modifier; c.Modifier.IsValid() {
				...
				case models.CriterionModifierMatchesRegex:
					...
					f.addWhere(fmt.Sprintf("%s IS NOT NULL AND %s IS NOT NULL AND regex_match(%s, ?)", pathColumn, basenameColumn, filepathColumn), c.Value)
				case models.CriterionModifierNotMatchesRegex:
					if _, err := regexp.Compile(c.Value); err != nil {
						f.setError(err)
						return
					...
					filepathColumn := fmt.Sprintf("%s || '%s' || %s", pathColumn, string(filepath.Separator), basenameColumn)
					f.addWhere(fmt.Sprintf("%s IS NULL OR %s IS NULL OR NOT regex_match(%s, ?)", pathColumn, basenameColumn, filepathColumn), c.Value)
				case models.CriterionModifierIsNull:
...
func getPathSearchClause(pathColumn, basenameColumn, p string, addWildcards, not bool) sqlClause {
	...
	filepathColumn := fmt.Sprintf("%s || '%s' || %s", pathColumn, string(filepath.Separator), basenameColumn)
	ret := makeClause(fmt.Sprintf("%s ILIKE ?", filepathColumn), p)
...
func resolutionCriterionHandler(resolution *models.ResolutionCriterionInput, heightColumn string, widthColumn string, addJoinFn func(f *filterBuilder)) criterionHandlerFunc {
	return func(ctx context.Context, f *filterBuilder) {
		if resolution != nil && resolution.Value.IsValid() {
			...
			widthHeight := fmt.Sprintf("LEAST(%s, %s)", widthColumn, heightColumn)
	...
	return func(ctx context.Context, f *filterBuilder) {
		if criterion != nil {
			if criterion.Modifier == models.CriterionModifierExcludes {
				...
				// need to use actual join table name for this
				// <primaryTable>.id NOT IN (select <joinTable>.<primaryFK> from <joinTable> where <joinTable>.<foreignFK> in <values>)
				whereClause := utils.StrFormat("{primaryTable}.id NOT IN (SELECT {joinTable}.{primaryFK} from {joinTable} where {joinTable}.{stringColumn} ILIKE ?)",
					utils.StrFormatMap{
						"primaryTable": m.primaryTable,
						"joinTable":    m.joinTable,
						"primaryFK":    m.primaryFK,
...
func getHierarchicalValues(ctx context.Context, values []string, table, relationsTable, parentFK string, childFK string, depth *int, parenthesis bool) (string, error) {
	...
	if depthVal == 0 {
		...
		if valid {
			...
			if parenthesis {
				values = "(" + values + ") AS v(column1, column2)"
			}
			return values, nil
		}
	}
...
`, withClauseMap)
	...
	query := fmt.Sprintf("WITH RECURSIVE %s SELECT 'VALUES' || STRING_AGG('(' || root_id || ', ' || item_id || ')'::TEXT, ',') AS val FROM items", withClause)
	...
	if parenthesis {
		valuesClause.String = "(" + valuesClause.String + ") AS v(column1, column2)"
	}
...
func (h *stashIDsCriterionHandler) handle(ctx context.Context, f *filterBuilder) {
	...
	case models.CriterionModifierEquals:
		...
		for _, id := range h.c.StashIDs {
			clauses = append(clauses, makeClause(fmt.Sprintf("%s.stash_id = '?'::uuid", t), id))
		}
		f.whereClauses = append(f.whereClauses, orClauses(clauses...))
	case models.CriterionModifierNotEquals:
		var clauses []sqlClause
