		...
		if c != nil {
			...
			if modifier := c.Modifier; c.Modifier.IsValid() {
				...
				case models.CriterionModifierEquals:
					...
					clause2 := makeClause(folderPathColumn+" ILIKE ?", c.Value)
					f.whereClauses = append(f.whereClauses, orClauses(clause, clause2))
				case models.CriterionModifierNotEquals:
					addWildcards = false
					not = true
					clause := getPathSearchClause(pathColumn, basenameColumn, c.Value, addWildcards, not)
					clause2 := makeClause(folderPathColumn+" NOT ILIKE ?", c.Value)
					f.whereClauses = append(f.whereClauses, orClauses(clause, clause2))
				case models.CriterionModifierMatchesRegex:
					...
					clause := makeClause(fmt.Sprintf("%s IS NOT NULL AND %s IS NOT NULL AND regex_match(%s, ?)", pathColumn, basenameColumn, filepathColumn), c.Value)
					clause2 := makeClause(fmt.Sprintf("%s IS NOT NULL AND regex_match(%[1]s, ?)", folderPathColumn), c.Value)
					f.whereClauses = append(f.whereClauses, orClauses(clause, clause2))
				case models.CriterionModifierNotMatchesRegex:
					...
					f.addWhere(fmt.Sprintf("%s IS NULL OR NOT regex_match(%[1]s, ?)", folderPathColumn), c.Value)
...
func (qb *galleryFilterHandler) missingCriterionHandler(isMissing *string) criterionHandlerFunc {
	return func(ctx context.Context, f *filterBuilder) {
		if isMissing != nil && *isMissing != "" {
			...
			case "date":
				f.addWhere("galleries.date IS NULL")
			case "tags":
				galleryRepository.tags.join(f, "tags_join", "galleries.id")
				f.addWhere("tags_join.gallery_id IS NULL")
			default:
				f.addWhere("(galleries." + *isMissing + " IS NULL OR TRIM(CAST(galleries." + *isMissing + " AS TEXT)) = '')")
			}
		}
	}
}
	...
	return func(ctx context.Context, f *filterBuilder) {
		if performerfavorite != nil {
			...
			if *performerfavorite {
				...
				f.addWhere("performers.favorite = true")
			} else {
...
GROUP BY performers_galleries.gallery_id HAVING SUM(performers.favorite) = false)`, "nofaves", "galleries.id = nofaves.id")
...
func (qb *galleryFilterHandler) averageResolutionCriterionHandler(resolution *models.ResolutionCriterionInput) criterionHandlerFunc {
	return func(ctx context.Context, f *filterBuilder) {
		if resolution != nil && resolution.Value.IsValid() {
			...
			const widthHeight = "avg(LEAST(image_files.width, image_files.height))"
