...
func (r *mutationResolver) ImageDestroy(ctx context.Context, input models.ImageDestroyInput) (ret bool, err error) {
	...
	if err := r.withTxn(ctx, func(repo models.Repository) error {
		...
		return image.Destroy(i, repo, fileDeleter, utils.IsTrue(input.DeleteGenerated), utils.IsTrue(input.DeleteFile))
	}); err != nil {
...
func (r *mutationResolver) ImagesDestroy(ctx context.Context, input models.ImagesDestroyInput) (ret bool, err error) {
	...
	if err := r.withTxn(ctx, func(repo models.Repository) error {
		...
		for _, imageID := range imageIDs {
			...
			if err := image.Destroy(i, repo, fileDeleter, utils.IsTrue(input.DeleteGenerated), utils.IsTrue(input.DeleteFile)); err != nil {
				return err
			}
		}
