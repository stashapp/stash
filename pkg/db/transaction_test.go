...
// 						if err := conn.Scene.Create(ctx, scene, nil); err != nil {
// 							return err
// 						}

// 						if err := conn.Scene.Destroy(ctx, scene.ID); err != nil {
// 							return err
// 						}
// 					}
...
func TestConcurrentReadTxn(t *testing.T) {
	...
	go func() {
		defer wg.Done()
		if err := txn.WithReadTxn(ctx, conn, func(ctx context.Context) error {
			scene := &models.Scene{
				Title: "test",
			}
	...
	go func() {
		defer wg.Done()
		_ = txn.WithReadTxn(ctx, conn, func(ctx context.Context) error {
...
func TestConcurrentExclusiveAndReadTxn(t *testing.T) {
	...
	go func() {
		defer wg.Done()
		if err := txn.WithTxn(ctx, conn, func(ctx context.Context) error {
			scene := &models.Scene{
				Title: "test",
			}
	...
	go func() {
		...
		_ = txn.WithReadTxn(ctx, conn, func(ctx context.Context) error {
			...
			if _, err := conn.Scene.Find(ctx, sceneIDs[sceneIdx1WithPerformer]); err != nil {
				t.Errorf("unexpected error: %v", err)
				return err
			}
...
// 					for ll := 0; ll < innerLoops; ll++ {
// 						if _, err := conn.Scene.Find(ctx, sceneIDs[ll%totalScenes]); err != nil {
// 							return err
// 						}
// 					}
// 					time.Sleep(sleepTime)
