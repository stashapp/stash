...
func TestSavedFilterFind(t *testing.T) {
	withTxn(func(ctx context.Context) error {
		savedFilter, err := conn.SavedFilter.Find(ctx, savedFilterIDs[savedFilterIdxImage])
...
func TestSavedFilterFindByMode(t *testing.T) {
	withTxn(func(ctx context.Context) error {
		savedFilters, err := conn.SavedFilter.FindByMode(ctx, models.FilterModeScenes)
...
func TestSavedFilterDestroy(t *testing.T) {
	...
	withTxn(func(ctx context.Context) error {
		...
		err := conn.SavedFilter.Create(ctx, &newFilter)
	...
	withTxn(func(ctx context.Context) error {
		return conn.SavedFilter.Destroy(ctx, id)
	})

	// now try to find it
	withTxn(func(ctx context.Context) error {
		found, err := conn.SavedFilter.Find(ctx, id)
		if err == nil {
			assert.Nil(t, found)
		}
...
func TestSavedFilterFindDefault(t *testing.T) {
	withTxn(func(ctx context.Context) error {
		def, err := conn.SavedFilter.FindDefault(ctx, models.FilterModeScenes)
		if err == nil {
...
func TestSavedFilterSetDefault(t *testing.T) {
	...
	withTxn(func(ctx context.Context) error {
		err := conn.SavedFilter.SetDefault(ctx, &models.SavedFilter{
	...
	var defID int
	withTxn(func(ctx context.Context) error {
		def, err := conn.SavedFilter.FindDefault(ctx, models.FilterModeMovies)
		if err == nil {
			defID = def.ID
	...
	// destroy it again
	withTxn(func(ctx context.Context) error {
		return conn.SavedFilter.Destroy(ctx, defID)
	})
}
