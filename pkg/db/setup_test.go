...
func populateDB() error {
	if err := withTxn(func(ctx context.Context) error {
		...
		if err := createMovies(ctx, conn.Movie, moviesNameCase, moviesNameNoCase); err != nil {
			return fmt.Errorf("error creating movies: %s", err.Error())
		}

		if err := createTags(ctx, conn.Tag, tagsNameCase, tagsNameNoCase); err != nil {
		...
		if err := createStudios(ctx, conn.Studio, studiosNameCase, studiosNameNoCase); err != nil {
...
func createGalleries(ctx context.Context, n int) error {
	gqb := conn.TxnRepository().Gallery
	fqb := conn.File
