...
func loadSceneRelationships(ctx context.Context, expected models.Scene, actual *models.Scene) error {
	if expected.GalleryIDs.Loaded() {
		if err := actual.LoadGalleryIDs(ctx, conn.Scene); err != nil {
			return err
		}
	}
	if expected.TagIDs.Loaded() {
		if err := actual.LoadTagIDs(ctx, conn.Scene); err != nil {
			return err
		}
	}
	if expected.PerformerIDs.Loaded() {
		if err := actual.LoadPerformerIDs(ctx, conn.Scene); err != nil {
			return err
		}
	}
	if expected.Movies.Loaded() {
		if err := actual.LoadMovies(ctx, conn.Scene); err != nil {
			return err
		}
	}
	if expected.StashIDs.Loaded() {
		if err := actual.LoadStashIDs(ctx, conn.Scene); err != nil {
			return err
		}
	}
	if expected.Files.Loaded() {
		if err := actual.LoadFiles(ctx, conn.Scene); err != nil {
			return err
		}
	}
...
func TestSceneQuery(t *testing.T) {
	...
	for _, tt := range tests {
		runWithRollbackTxn(t, tt.name, func(t *testing.T, ctx context.Context) {
			...
			results, err := conn.Scene.Query(ctx, models.SceneQueryOptions{
...
func verifyScenesDuration(t *testing.T, durationCriterion models.IntCriterionInput) {
	withTxn(func(ctx context.Context) error {
		...
		for _, scene := range scenes {
			if err := scene.LoadPrimaryFile(ctx, conn.File); err != nil {
...
func createScene(ctx context.Context, width int, height int) (*models.Scene, error) {
	...
	if err := conn.File.Create(ctx, sceneFile); err != nil {
		return nil, err
	}

	scene := &models.Scene{}

	if err := conn.Scene.Create(ctx, scene, []file.ID{sceneFile.ID}); err != nil {
		return nil, err
	}
