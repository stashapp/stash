...
func loadImageRelationships(ctx context.Context, expected models.Image, actual *models.Image) error {
	if expected.GalleryIDs.Loaded() {
		if err := actual.LoadGalleryIDs(ctx, conn.Image); err != nil {
			return err
		}
	}
	if expected.TagIDs.Loaded() {
		if err := actual.LoadTagIDs(ctx, conn.Image); err != nil {
			return err
		}
	}
	if expected.PerformerIDs.Loaded() {
		if err := actual.LoadPerformerIDs(ctx, conn.Image); err != nil {
			return err
		}
	}
	if expected.Files.Loaded() {
		if err := actual.LoadFiles(ctx, conn.Image); err != nil {
			return err
		}
	}
...
func verifyImagesResolution(t *testing.T, resolution models.ResolutionEnum) {
	withTxn(func(ctx context.Context) error {
		...
		for _, image := range images {
			if err := image.LoadPrimaryFile(ctx, conn.File); err != nil {
...
func TestImageQueryPerformers(t *testing.T) {
	...
	for _, tt := range tests {
		runWithRollbackTxn(t, tt.name, func(t *testing.T, ctx context.Context) {
			...
			results, err := conn.Image.Query(ctx, models.ImageQueryOptions{
				ImageFilter: &models.ImageFilterType{
