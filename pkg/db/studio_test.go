...
func TestStudioDestroyParent(t *testing.T) {
	...
	if err := withTxn(func(ctx context.Context) error {
		createdParent, err := createStudio(ctx, conn.Studio, parentName, nil)
		if err != nil {
			return fmt.Errorf("Error creating parent studio: %s", err.Error())
		}

		parentID := createdParent.ID
		createdChild, err := createStudio(ctx, conn.Studio, childName, &parentID)
		if err != nil {
...
func TestStudioUpdateStudioImage(t *testing.T) {
	if err := withTxn(func(ctx context.Context) error {
		...
		created, err := createStudio(ctx, conn.Studio, name, nil)
		if err != nil {
...
func verifyStudiosSceneCount(t *testing.T, sceneCountCriterion models.IntCriterionInput) {
	withTxn(func(ctx context.Context) error {
		...
		for _, studio := range studios {
			sceneCount, err := conn.Scene.CountByStudioID(ctx, studio.ID)
			if err != nil {
				return err
			}
...
func TestStudioQueryAlias(t *testing.T) {
	...
	verifyFn := func(ctx context.Context, studio *models.Studio) {
		t.Helper()
		aliases, err := conn.Studio.GetAliases(ctx, studio.ID)
		if err != nil {
