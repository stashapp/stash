...
func loadGalleryRelationships(ctx context.Context, expected models.Gallery, actual *models.Gallery) error {
	if expected.SceneIDs.Loaded() {
		if err := actual.LoadSceneIDs(ctx, conn.Gallery); err != nil {
			return err
		}
	}
	if expected.TagIDs.Loaded() {
		if err := actual.LoadTagIDs(ctx, conn.Gallery); err != nil {
			return err
		}
	}
	if expected.PerformerIDs.Loaded() {
		if err := actual.LoadPerformerIDs(ctx, conn.Gallery); err != nil {
			return err
		}
	}
	if expected.Files.Loaded() {
		if err := actual.LoadFiles(ctx, conn.Gallery); err != nil {
			return err
		}
	}
...
func TestGalleryQueryPerformers(t *testing.T) {
	...
	for _, tt := range tests {
		runWithRollbackTxn(t, tt.name, func(t *testing.T, ctx context.Context) {
			...
			results, _, err := conn.Gallery.Query(ctx, &models.GalleryFilterType{
...
func TestGalleryQueryPerformerTags(t *testing.T) {
	...
	for _, tt := range tests {
		runWithRollbackTxn(t, tt.name, func(t *testing.T, ctx context.Context) {
			...
			results, _, err := conn.Gallery.Query(ctx, tt.filter, tt.findFilter)
			if (err != nil) != tt.wantErr {
