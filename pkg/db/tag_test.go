...
func TestTagQueryAlias(t *testing.T) {
	...
	verifyFn := func(ctx context.Context, tag *models.Tag) {
		aliases, err := conn.Tag.GetAliases(ctx, tag.ID)
		if err != nil {
...
func TestTagMerge(t *testing.T) {
	...
	if err := withRollbackTxn(func(ctx context.Context) error {
		...
		s, err := conn.Scene.Find(ctx, sceneIDs[sceneIdxWithTwoTags])
		if err != nil {
			return err
		}
		if err := s.LoadTagIDs(ctx, conn.Scene); err != nil {
			return err
		}
		...
		imageTagIDs, err := conn.Image.GetTagIDs(ctx, imageIDs[imageIdxWithTwoTags])
		if err != nil {
			return err
		}
		...
		g, err := conn.Gallery.Find(ctx, galleryIDs[galleryIdxWithTwoTags])
		if err != nil {
			return err
		}

		if err := g.LoadTagIDs(ctx, conn.Gallery); err != nil {
			return err
		}
		...
		performerTagIDs, err := conn.Performer.GetTagIDs(ctx, performerIDs[performerIdxWithTwoTags])
		if err != nil {
			return err
		}
