...
func (t *ScanTask) scanScene() *models.Scene {
	...
	if err := t.TxnManager.WithTxn(context.TODO(), func(r models.Repository) error {
		qb := r.Scene()
		s, err := qb.FindByPath(t.FilePath)
		if err != nil {
			return err
		}
		...
		if s != nil {
			...
			if modified || !s.Size.Valid {
				s, err = t.rescanScene(qb, s, fileModTime)
				if err != nil {
					return err
				}
			}
			...
			if !s.Format.Valid {
				...
				if _, err = scene.UpdateFormat(qb, s.ID, string(container)); err != nil {
					return err
				}
			}
			...
			if !s.OSHash.Valid {
				...
				if _, err := scene.UpdateOSHash(qb, s.ID, oshash); err != nil {
					return err
				}
			}
			...
			if t.calculateMD5 && !s.Checksum.Valid {
				...
				if _, err := scene.UpdateChecksum(qb, s.ID, checksum); err != nil {
					return err
				}
			}
		...
		if s != nil {
			...
			retScene, err = qb.Create(newScene)
			if err != nil {
				return err
			}
		}
...
func (t *ScanTask) rescanScene(qb models.SceneReaderWriter, s *models.Scene, fileModTime time.Time) (*models.Scene, error) {
	logger.Infof("%s has been updated: rescanning", t.FilePath)
	...
	ret, err := qb.Update(scenePartial)
	if err != nil {
		logger.Error(err.Error())
		return nil, err
	}
