...
func makeConfigGeneralResult() *ConfigGeneralResult {
	...
	return &ConfigGeneralResult{
		...
		S3Endpoint:                    config.GetS3Options().Endpoint,
		S3AccessKey:                   config.GetS3Options().AccessKeyID,
		S3SecretKey:                   config.GetS3Options().SecretAccessKey,
		S3Bucket:                      config.GetS3Options().Bucket,
		S3BlobsPrefix:                 config.GetS3Options().BlobsPrefix,
		S3Region:                      config.GetS3Options().Region,
		S3UseSsl:                      config.GetS3Options().UseSSL,
