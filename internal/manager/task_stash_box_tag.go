...
func (t *StashBoxBatchTagTask) Description() string {
	...
	return fmt.Sprint("Uknown tagging task from stash-box")
}
	...
	if t.refresh {
		...
		txnErr := txn.WithReadTxn(ctx, instance.Repository, func(ctx context.Context) error {
			if !t.performer.StashIDs.Loaded() {
				t.performer.LoadStashIDs(ctx, instance.Repository.Performer)
			}
...
func (t *StashBoxBatchTagTask) stashBoxStudioTag(ctx context.Context) {
	...
	if t.refresh {
		...
		txnErr := txn.WithReadTxn(ctx, instance.Repository, func(ctx context.Context) error {
			if !t.studio.StashIDs.Loaded() {
				t.studio.LoadStashIDs(ctx, instance.Repository.Studio)
			}
		...
		if studioID != "" {
			studio, err = client.FindStashBoxStudio(ctx, studioID)
		}
	} else {
		var name string
		if t.name != nil {
	...
	if studio != nil {
		if t.studio != nil {
			...
			if studio.Name != "" && !excluded["name"] {
				partial.Name = models.NewOptionalString(studio.Name)

			}
			if studio.URL != nil && !excluded["url"] {
				partial.URL = models.NewOptionalString(*studio.URL)
			}
			if !t.refresh {
				partial.StashIDs = &models.UpdateStashIDs{
					StashIDs: []models.StashID{
			...
			txnErr := txn.WithTxn(ctx, instance.Repository, func(ctx context.Context) error {
				r := instance.Repository
				_, err := r.Studio.UpdatePartial(ctx, t.studio.ID, partial)

				if len(studio.Images) > 0 && !excluded["image"] {
					image, err := utils.ReadImageFromURL(ctx, studio.Images[0])
					if err != nil {
						return err
					}
					err = r.Studio.UpdateImage(ctx, t.studio.ID, image)
					if err != nil {
						return err
					}
				}
				...
				if err == nil {
					...
					logger.Infof("Updated studio %s", name)
				}
				return err
			})
			if txnErr != nil {
				logger.Warnf("failure to execute partial update of studio: %v", err)
			}
			...
			if studio.URL != nil {
				newStudio.URL = *studio.URL
			}

			err := txn.WithTxn(ctx, instance.Repository, func(ctx context.Context) error {
				r := instance.Repository
				err := r.Studio.Create(ctx, &newStudio)
				if err != nil {
					return err
				}

				if len(studio.Images) > 0 {
					image, imageErr := utils.ReadImageFromURL(ctx, studio.Images[0])
					if imageErr != nil {
						return imageErr
					}
					err = r.Studio.UpdateImage(ctx, newStudio.ID, image)
				}
				return err
			})
			if err != nil {
				logger.Errorf("Failed to save studio %s: %s", *t.name, err.Error())
			} else {
				logger.Infof("Saved studio %s", *t.name)
			}
		}
	} else {
		var name string
